...
public class OFMessageUtils {
	...
	public static String msgToString(OFMessage m) {
		if (m == null)
			return "null";
		
		if (m.getVersion() != OFVersion.OF_14) {
			return m.toString();
		}
		StringBuilder sb = new StringBuilder("m=[xid=" + m.getXid() + ", type="
				+ m.getType());
		if (m.getType() == OFType.PACKET_IN) {
			OFPacketIn pktIn = (OFPacketIn) m;
			sb.append(", match=" + pktIn.getMatch());
			sb.append(", reason=" + pktIn.getReason());
			sb.append(", cookie=" + pktIn.getCookie());
			sb.append(", buffer=" + pktIn.getBufferId());
			sb.append(", totalLen=" + pktIn.getTotalLen());
		} else if (m.getType() == OFType.PORT_STATUS) {
			OFPortStatus ps = (OFPortStatus) m;
			sb.append(", reason=" + ps.getReason());
			sb.append(", desc=" + ps.getDesc());
		} else if (m.getType() == OFType.PACKET_OUT) {
			OFPacketOut ofpo = (OFPacketOut) m;
			sb.append(", inPort=" + ofpo.getInPort());
			sb.append(", actions=" + ofpo.getActions());
		}
